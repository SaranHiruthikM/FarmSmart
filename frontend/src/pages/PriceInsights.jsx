...
const PriceInsights = () => {
    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            <div className="flex flex-col gap-8 bg-white p-10 rounded-[3rem] shadow-xl shadow-neutral-light border border-neutral-light/50 relative overflow-hidden group">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-5xl font-black text-text-dark tracking-tighter uppercase italic leading-none">Market Insights</h1>
                        </div>
                    ...
                    <button
                        ...
                        className="group flex items-center gap-4 px-8 py-4 bg-gradient-to-br from-primary via-green-600 to-green-700 text-white font-black text-[12px] uppercase tracking-[0.25em] rounded-2xl hover:shadow-[0_20px_40px_-10px_rgba(22,101,52,0.4)] transition-all hover:-translate-y-1.5 active:scale-95 disabled:opacity-50 disabled:translate-y-0 relative overflow-hidden"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
                    ...
                    {/* Range Selectors Move inside the grid to make it 4 columns */}
                    <div className="space-y-1.5 pt-1">
                        <label className="text-[10px] font-black text-accent uppercase tracking-[0.2em] ml-1">Range</label>
                        <div className="bg-neutral-light/30 p-1.5 rounded-2xl flex items-center gap-1">
                            {["30", "180", "365"].map((range) => (
                                <button
                                    key={range}
                                    onClick={() => setTimeFilter(range)}
                                    className={`flex-1 py-2 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest ${timeFilter === range ? 'bg-white text-primary shadow-sm' : 'text-accent hover:bg-white/50'}`}
                                >
            ...
            {/* AI Advisor Chat Interface */}
            {
                showForecast && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-in fade-in duration-300">
                        <div className="bg-white w-full max-w-2xl h-[600px] rounded-[3rem] shadow-2xl border border-neutral-light overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
                            {/* Chat Header */}
                            <div className="bg-gradient-to-r from-primary via-green-600 to-green-700 p-8 text-white relative">
                                ...
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        <h2 className="text-2xl font-black uppercase tracking-tight">Price Predictor</h2>
...
const PriceCard = ({ label, value, unit, icon: Icon, color, bg, isText = false }) => (
    <div className="bg-white p-8 rounded-[2.5rem] border border-neutral-light/50 shadow-sm flex flex-col justify-between space-y-6 h-full transition-all duration-500 hover:shadow-xl hover:shadow-neutral-light/50 hover:translate-y-[-4px] group border-b-4 border-b-transparent hover:border-b-primary/50">
        ...
        <div>
            <p className="text-[11px] font-black text-accent/50 uppercase tracking-[0.25em] mb-2">{label}</p>
            <div className="flex items-baseline gap-1.5">
                <span className={`font-black tracking-tighter ${isText ? 'text-2xl text-text-dark line-clamp-1 break-all uppercase italic' : 'text-4xl ' + color}`} title={isText ? value : undefined}>
                    {value}
                </span>
                {!isText && unit && <span className="text-[10px] text-accent font-black uppercase tracking-widest bg-neutral-light/30 px-2 py-1 rounded-lg ml-1">{unit}</span>}
            </div>
        </div>
    </div>
);
