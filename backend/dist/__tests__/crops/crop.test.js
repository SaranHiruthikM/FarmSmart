...
describe('CROP CONTROLLER - COMPREHENSIVE TEST SUITE', () => {
    // ====== SETUP & TEARDOWN ======
    beforeAll(async () => {
        await (0, testSetup_1.connect)();
        const { default: appModule } = await Promise.resolve().then(() => __importStar(require('../../src/app')));
        app = appModule;
    });
    beforeEach(async () => {
        await Crop_1.Crop.deleteMany({});
        await User_1.default.deleteMany({});
        // Create base users
        const farmerUser = await User_1.default.create(validFarmer);
        const buyerUser = await User_1.default.create(validBuyer);
    ...
    afterAll(async () => {
        await (0, testSetup_1.clearDatabase)();
        await (0, testSetup_1.closeDatabase)();
    });
    ...
    describe('GET /crops - List Crops', () => {
        ...
        it('✓ Should list all active crops', async () => {
            const res = await (0, supertest_1.default)(app).get('/crops');
        ...
        it('✓ Should not list inactive crops', async () => {
            const res = await (0, supertest_1.default)(app).get('/crops');
            expect(res.status).toBe(200);
            const inactiveCrops = res.body.filter((crop) => !crop.isActive);
    ...
    describe('GET /crops/:id - Get Single Crop', () => {
        ...
        it('✓ Should retrieve crop by ID', async () => {
            const res = await (0, supertest_1.default)(app).get(`/crops/${cropId}`);
            expect(res.status).toBe(200);
        ...
        it('✗ Should return 404 for non-existent crop ID', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get(`/crops/${fakeId}`);
            expect(res.status).toBe(404);
            expect(res.body.message).toBe('Crop not found');
        });
        it('✗ Should return 404 for invalid crop ID format', async () => {
            const res = await (0, supertest_1.default)(app).get('/crops/invalidid');
            expect(res.status).toBe(400);
        });
        ...
        it('✓ Should retrieve inactive crop', async () => {
            const inactiveCrop = await Crop_1.Crop.create({
                ...validCrop,
                farmerId: farmerId,
                isActive: false,
            });
            const res = await (0, supertest_1.default)(app).get(`/crops/${inactiveCrop._id}`);
            expect(res.status).toBe(200);
    ...
    describe('GET /crops/my - Get Farmer\'s Crops', () => {
        let anotherFarmerId;
        let anotherFarmerToken;
        beforeEach(async () => {
            // Create another farmer
            const anotherFarmer = await User_1.default.create({
        ...
        it('✓ Should retrieve only authenticated farmer\'s crops', async () => {
            ...
            expect(Array.isArray(res.body)).toBe(true);
            expect(res.body.length).toBe(2);
            expect(res.body.every((crop) => crop.farmerId.toString() === farmerId)).toBe(true);
        });
        it('✓ Should return empty array when farmer has no crops', async () => {
            const thirdFarmer = await User_1.default.create({
        ...
        it('✗ Should reject unauthenticated request', async () => {
            const res = await (0, supertest_1.default)(app).get('/crops/my');
            expect(res.status).toBe(401);
        });
    ...
    describe('DELETE /crops/:id - Delete Crop', () => {
        ...
        it('✓ Should delete crop successfully', async () => {
            ...
            const deletedCrop = await Crop_1.Crop.findById(cropId);
        ...
        it('✗ Should reject unauthenticated delete request', async () => {
            const res = await (0, supertest_1.default)(app).delete(`/crops/${cropId}`);
            expect(res.status).toBe(401);
            // Verify crop still exists
            const crop = await Crop_1.Crop.findById(cropId);
            expect(crop).not.toBeNull();
    ...
    describe('Integration Tests - Multiple Operations', () => {
        ...
        it('✓ Should create crop and retrieve via GET /crops', async () => {
            const createRes = await (0, supertest_1.default)(app)
                ...
                .set('Authorization', `Bearer ${farmerToken}`)
                .send(validCrop);
            const listRes = await (0, supertest_1.default)(app).get('/crops');
            expect(listRes.status).toBe(200);
            const foundCrop = listRes.body.find((crop) => crop._id === createRes.body._id);
...
function generateToken(userId, role = 'FARMER') {
    ...
    return jwt.sign({ id: userId, role }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '7d' });
}
